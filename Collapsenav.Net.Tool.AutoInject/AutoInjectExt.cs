...
public static class AutoInjectExt
{
    ...
    {
        // 首先获取 CustomerTypes , 但是剔除 Collapsenav.Net.Tool 的 types
        ...
        // 获取存在 AutoInject 标记属性的Types，以待后续进行注册
        var autoInjectTypesTuple = types.Select(item => (ServiceType: item, InjectTypes: item.AttrMemberTypes<AutoInjectAttribute>()))
            .Where(item=>item.InjectTypes.NotEmpty());
        // 获取被标记了 AutoInject 的 types
        var autoInjectTypes = autoInjectTypesTuple.SelectMany(item => item.InjectTypes)
        // 移除已经手动注册过的 type
